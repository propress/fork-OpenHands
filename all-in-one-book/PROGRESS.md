...
### 从哪里继续
从第10章 ch10-evolution.md 开始写作。序章至第9章已完成。

### 交接备忘
- 项目版本 v1.5.0，处于 V0→V1 迁移期（V0 将于 2026.4 废弃）
- 总 commit 约 6370 个，时间跨度 2024.3 - 2026.3
- 第10章演进史需要阅读 commit 历史（约 6370 个），按里程碑划分阶段
  - 建议用 `git log --oneline --reverse | head -100` 先看最早的 commits
  - 按 tag 划分阶段：`git tag --sort=creatordate`
  - 重点关注架构变化：V0 基础搭建 → Agent 框架 → 多 Runtime → V1 迁移
- 第11章端到端追踪建议选"修复一个 GitHub Issue"作为典型场景
  - 从用户在浏览器输入开始，到 Agent 完成修复、推送代码
  - 引用前面每一章的具体机制
- 已完成的9章覆盖了核心系统，风格和深度已稳定，续写需保持一致
